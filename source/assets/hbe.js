export function initHBE() {
  ...
  const submitBtn = document.getElementById("hbeSubmit");
  ...
  function setLoading(loading) {
    const inp = document.getElementById("hbePass");
    if (inp) inp.disabled = loading;
    if (submitBtn) {
      submitBtn.disabled = loading;
      if (loading) submitBtn.classList.add("hbe-btn--loading");
      else submitBtn.classList.remove("hbe-btn--loading");
    }
  }
  ...
  async function attemptDecrypt() {
    ...
    const keyMaterial = await getKeyMaterial(input.value);
  ...
  function hbeLoader() {
    ...
    if (submitBtn) {
      submitBtn.addEventListener("click", attemptDecrypt);
    }
  }
